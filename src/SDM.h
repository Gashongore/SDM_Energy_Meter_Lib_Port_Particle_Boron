...
const int NOT_A_PIN =2; //Hardware flow control RTS for Serial1 is optionally available on pinsD2(RTS)
...
class SDM {
  public:                                                                        //  on avr
    SDM(long baud = SDM_UART_BAUD, int dere_pin = DERE_PIN, int config = SDM_UART_CONFIG);
  ...
  private:
    ...
    void sdm_flush(unsigned long _flushtime = 0);                                   //  read serial if any old data is available or for a given time in ms
